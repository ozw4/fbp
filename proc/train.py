...
class MaskedSegyGather(Dataset):
	def __init__(
		...
		augment_freq_prob: float = 0.0,  # 学習: 0.5 など / 検証: 0.0
		augment_freq_kinds: tuple[str, ...] = ('bandpass', 'lowpass', 'highpass'),
		augment_freq_band: tuple[float, float] = (
			0.05,
			0.45,
		),  # 正規化周波数(0..1, Nyquist=1)
		augment_freq_width: tuple[float, float] = (0.10, 0.35),  # bandpass幅(正規化)
		augment_freq_roll: float = 0.02,  # バンド端のコサイン立ち上がり幅
	...
	@staticmethod
	def _cosine_ramp(
		x: np.ndarray, a: float, b: float, invert: bool = False
	) -> np.ndarray:
		"""[a,b]で0→1(または1→0)のコサインランプ。a>=bならステップ幅0として丸める。"""
		if b <= a:
	...
	def _make_freq_mask(
		...
		"""rFFT長 n_rfft 用の滑らかな周波数マスクを生成（0..Nyquist を 0..1 で正規化）。
		kind: 'bandpass' | 'lowpass' | 'highpass'
		"""
		f = np.linspace(0.0, 1.0, n_rfft, dtype=np.float32)  # 0..Nyquist 正規化
		m = np.ones_like(f, dtype=np.float32)

		if kind == 'bandpass':
			assert f_lo is not None and f_hi is not None and f_hi > f_lo
			bw = max(f_hi - f_lo, 1e-4)
			r = max(roll, 0.05 * bw)  # バンド幅に対して相対的に少し広めに
			# 立ち上がり・立ち下がりをコサインでスムーズに
			up = self._cosine_ramp(
				f, max(0.0, f_lo - r), min(1.0, f_lo + r), invert=False
			)
			dn = self._cosine_ramp(
				f, max(0.0, f_hi - r), min(1.0, f_hi + r), invert=True
			)
			m = up * dn
		elif kind == 'lowpass':
	...
	def _apply_freq_augment(self, x_hw: np.ndarray) -> np.ndarray:
		"""(H,W) の全トレースに同一マスクを適用（空間コヒーレンス保持）。
		位相は rFFT/irFFTで保存（実質ゼロ位相）。出力は float32。
		"""
		H, W = x_hw.shape
		# ランダムに種類を選ぶ
		kind = random.choice(self.augment_freq_kinds)

		# バンド設定（正規化0..1）
		lo_min, hi_max = self.augment_freq_band
		if kind == 'bandpass':
			min_w, max_w = self.augment_freq_width
			# 帯域幅と下端をランダムに
		...
		# マスク作成
		n_rfft = W // 2 + 1
		mask = self._make_freq_mask(
			n_rfft, kind, f_lo, f_hi, self.augment_freq_roll
		)  # (n_rfft,)

		# rFFT → マスク乗算 → irFFT
		X = np.fft.rfft(x_hw, axis=1)  # (H, n_rfft), complex
		Y = X * mask[None, :]  # 振幅整形（位相そのまま）
		y = np.fft.irfft(Y, n=W, axis=1).astype(np.float32)  # (H,W), real

		if self.augment_freq_restandardize:
			# per-trace 標準化でダイナミクスを安定
