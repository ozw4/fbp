...
criterion = (
        make_fb_seg_mse_criterion() if task == 'fb_seg' else make_criterion(cfg.loss)
)
        ...
        if task == 'recon':
                ...
                if utils.is_main_process():
                        ...
                        valid_writer.add_scalar(
                                'val_snr/valid_frac', snr_dict['valid_frac'], epoch
                        )
                ...
                synthe_metrics = {'mse': 0.0, 'mae': 0.0, 'psnr': 0.0}
                save_flag = False
