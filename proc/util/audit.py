...
def assert_meta_shapes(meta: dict, B: int, H: int) -> None:
	...
	offs = meta['offsets']
	dt = meta['dt_sec']
	assert offs.ndim == 2 and offs.shape == (B, H), (
		f'offsets shape {offs.shape}, expected {(B, H)}'
	)
	assert torch.isfinite(offs).all(), 'offsets has NaN/Inf'
	assert dt.ndim in (1, 2) and dt.shape[0] == B, (
		f'dt_sec shape {dt.shape}, expected (B,) or (B,1)'
	)
...
def audit_offsets_and_mask_coverage(
	...
	for bi, (x, target, _, meta) in enumerate(loader):
		...
		nonmonotonic_shot += int(
			((diffs >= 0).all(dim=1) | (diffs <= 0).all(dim=1))
			.logical_not()
			.sum()
			.item()
		)
	...
	print(
		f'[AUDIT] dx[m] min/med/max: {stats["dx_min"]:.6g} / {stats["dx_med"]:.6g} / {stats["dx_max"]:.6g}'
	)
	print('[AUDIT] dx==0 count:       ', stats['dx_eq0'])
	print('[AUDIT] mask==0 traces:    ', stats['mask_all_zero_traces'])
	print(
		'[AUDIT] low coverage (<{:.2f}) valid traces: {}'.format(
			stats['cov_threshold'], stats['low_coverage_traces']
		)
	)
	print(
		'[AUDIT] NaN/Inf offsets elts:',
		stats['nan_off_elems'],
		'  NaN/Inf dt_sec elts:',
		stats['nan_dt_elems'],
	)
	print(
		'[AUDIT] shots all-equal offsets:',
		stats['all_equal_shots'],
		'  non-monotonic shots (mixed up/down):',
		stats['nonmonotonic_shots'],
	)
