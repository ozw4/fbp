# %%
# proc/test.inference_tta.py (tmp1 + cross-merge + pbar hit2)
"""Collect ALL logits per view in raw-global order, evaluate per view,
then **cross-merge** across domains/views, with tqdm pbar reporting Hit@2ms.
...
- For each (domain, view):
    ...
    * reconstruct a single (N_total_traces, W) array by averaging overlapped windows in **raw-global row order**
...
- After all views are processed:
    ...
    * do **cross-domain merge** in raw-global order
        ...
        - infer.cross_view_temps:   {shot_none:1.0, recv:1.0, ...}  # optional for PoE

Note: This variant intentionally **does not** add the offset_phys nor time input channels.
Input remains single-channel (amplitude) as in tmp1.
"""
...
# -------------------------
# Model helpers (1ch input; no offset/time inflations)
# -------------------------
...
def build_model_from_cfg(cfg) -> torch.nn.Module:
	...
	# keep tmp1 behavior: widen first conv padding for caformer
...
# -------------------------
# Deterministic enumerator (windows) with raw-global index
# -------------------------


class InferenceGatherWindows(MaskedSegyGather):
	"""Deterministic window enumerator for inference (no random aug).
	Provides raw-global row index (raw_idx) and total trace count (n_total).
	"""
	...
	def __getitem__(self, i: int):
		...
		return {
			...
			'offsets': torch.from_numpy(
				offsets
			),  # retained in meta only; not used as input here
...
def make_valid_loader_for_domain(
	...
	*,
	...
	def _collate(batch):
		...
		meta['n_total'] = batch[0]['n_total']  # identical across items
...
def eval_view(
	...
	N, W = logits_2d.shape
	pred_idx = logits_2d.argmax(axis=1).astype(np.int64)
...
def _weight(view_weights: dict[str, float] | None, d: str, v: str) -> float:
	# Priority: 'domain:view' → 'domain' → 'view' → 1.0
...
def eval_cross_poe(
	...
	*,
	...
	| None = None,  # keys like 'shot_none', 'recv', 'hflip', 'shot:hflip'
) -> dict:
	sel = []
	names = []
	for d, v in use:
...
# -------------------------
# Main: per-view runner (with pbar incremental Hit@2ms)
# -------------------------
...
def run_one_view(
	...
	for x, _t, _m, meta in loader:
		...
		# lightweight per-batch incremental metric for pbar
		with torch.no_grad():
		...
		if inc_valid > 0:
			h2 = inc_hits.get('hit@2', 0) / max(inc_valid, 1)
			pbar.set_postfix(h2=f'{h2:.3f}', n=inc_valid)
		pbar.update(1)
