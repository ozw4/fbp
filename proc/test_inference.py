...
from util import MaskedSegyGather, segy_collate, worker_init_fn

# loss.py からロバスト回帰だけ借ります
from util.loss import gaussian_prior_from_trend, robust_linear_trend_sections
	...
	with open(csv_path, 'w', newline='') as fcsv:
		...
		for batch_idx, batch in enumerate(loader):
			...
			for b in range(B):
				...
				ax3.plot(
					offsets[b].cpu().numpy(),
					(1.0 / s_tr.clamp_min(1e-6))[0].cpu().numpy(),
				)
