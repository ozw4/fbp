...
class MaskedSegyGather(Dataset):
	...
	def __getitem__(self, _=None):
		...
		x_masked = x.copy()
		if num_mask > 0:
			noise = np.random.normal(0.0, 1.0, size=(num_mask, x.shape[1])) * noise_std
			x_masked[mask_idx] = x_masked[mask_idx] + noise
...
def train_one_epoch(
	...
	for i, batch in enumerate(metric_logger.log_every(dataloader, print_freq, header)):
		...
		with autocast(device_type=device_type, enabled=use_amp):
			...
			total_loss = criterion(pred, x_orig, mask=mask_2d, reduction='mean')
...
def criterion(
	...
	*,
	...
	pred_mask = pred * mask if mask is not None else pred
	gt_mask = gt * mask if mask is not None else gt
	# mae loss
	loss_fn = torch.nn.L1Loss(reduction=reduction)
	return loss_fn(pred_mask, gt_mask)
